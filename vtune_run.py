...
def modify_local_chat_py(file_path, new_value):
    with open(file_path, "r") as file:
        content = file.read()
    content = re.sub(
        r"torch.randint\(1, 63, torch.Size\(\[\d+\]\)\)",
        f"torch.randint(1, 63, torch.Size([{new_value}]))",
        content,
    )
    with open(file_path, "w") as file:
        file.write(content)
...
for cpu_infer in new_cpu_infer_values:
    for expert_num in expert_nums:
        modify_config_yaml(
            "/data/yanfansun/ktrans/ktransformers/ktransformers/configs/config.yaml",
            cpu_infer,
        )
        modify_local_chat_py(
            "/data/yanfansun/ktrans/ktransformers/ktransformers/local_chat.py",
            expert_num,
        )
